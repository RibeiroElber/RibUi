/*!
 * Selectr 2.4.13
 * Selectr 2.4.14 - Optgroup in .add(data) 
 *      @data {Object} = Eg.: { text: 'OptGroup Text', children: [ {value:0, text :'Opt 0'}
                                                                 , {value:1, text :'Opt 1'}
                                                                 , {value:2, text :'Opt 2'}]}
...
(function(root, factory) {
       ...
       if (isset(this.config, "width")) {
           if (util.isInt(this.config.width)) {
               ...
               if (this.config.width === "auto") {
                   this.width = "100%";
               } else if (util.includes(this.config.width, "%") || util.includes(this.config.width, "px")) {
    ...
    * @param {object} data
    */
   Selectr.prototype.add = function(data, checkDuplicate, isGroup) {
       if (data) {
           this.data = this.data || [];
               ...
               if (checkDuplicate) {
                ...
                if (isset(data, "children")) {
                    ...
                    var group = util.createElement("ul", {
                        class: "selectr-optgroup",
                        role: "group",
                        html: "<li class='selectr-optgroup-label'>" + data.text + "</li>"
                    });

                    // Loop into options in group
                    util.each(data.children, function(i, opt) {
                        var o = this.add(opt, checkDuplicate, true);
                        optgroup.appendChild(o);
                        group.appendChild(createItem.call(this, o, opt));
                    }, this);                
                    ...
                    // if IsNot a groupItem create new item
                    if(!isGroup)
