...
(function(root, factory) {
       ...
       if (data.data && data.data.length) {
           ...
           each(data.data, function (rows) {
               ...
               each(rows, function (value) {
                   var td = createElement("td", { html: value });
                   tr.appendChild(td);
               });
   ...
   Columns.prototype.add = function (data) {
       ...
       each(this.dt.data, function (row, i) {
           if (data.data[i]) {
               ...
               if (!data.data[i] || !data.data[i].length) {
                  data.data[i] = "";
               }
       ...
       if(data.hasOwnProperty("style")){
         Object.assign(th.style, data.style)
       }

       if(data.hasOwnProperty("class")){
         th.classList.add(data.class)
       }
   ...
   Columns.prototype.sort = function (column, direction, init) {
       ...
       var top, btm;
       if ( direction == "desc" || (!direction && classList.contains(th, "asc"))) {
           ...
           if (this.table.tHead) {
               each(this.activeHeadings, function (cell, i) {
                   ...
                   if(this.options.columns)
                       he = this.options.columns.filter(function(f){return f.select.indexOf(i)>-1})[0]; //get header parameters
                      
                   //force width only if we don´t have class or width in header parameters
                   if (!cell.hasOwnProperty("class") && !(he && he.style && he.style.width)){
                      cell.style.width = cell.offsetWidth / this.rect.width * 100 + "%";
                   }
               }, this);
           } else {
               cells = [];
               ...
               each(cells, function (cell, i) {
                   widths.push(cell.offsetWidth / this.rect.width * 100);
               }, this);
